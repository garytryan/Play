    ...
    return Backbone.View.extend({
      ...
      makeControlList: function(klass, stage){
        var c = {
          ...
          left:        {controls: [{ name: 'left',        label: 'left',         type: 'text',  value: Math.round(klass.get('left')) },                          { name: 'left',        type: 'range', value: Math.round(klass.get('left')),                          attrs:[{ type: 'min', value: 0 }, {type: 'max', value: stage.width }, { type: 'step', value: 1}] }]},
          height:      {controls: [{ name: 'height',      label: 'height',        type: 'text',  value: Math.round(klass.get('scaleX') * klass.get('height')) }, { name: 'height',      type: 'range', value: Math.round(klass.get('scaleX') * klass.get('height')), attrs:[{ type: 'min', value: 0 }, {type: 'max', value: stage.width }, { type: 'step', value: 1}] }]},
          width:       {controls: [{ name: 'width',       label: 'width',       type: 'text',  value: Math.round(klass.get('scaleY') * klass.get('width')) },    { name: 'width',       type: 'range', value: Math.round(klass.get('scaleY') * klass.get('width')),    attrs:[{ type: 'min', value: 0 }, {type: 'max', value: stage.width }, { type: 'step', value: 1}] }]},
          points:      {controls: [{ name: 'numPoints',   label: 'points',       type: 'text',  value: klass.get('numPoints') },                                 { name: 'numPoints',   type: 'range', value: klass.get('numPoints'),                                    attrs:[{ type: 'min', value: 2 }, {type: 'max', value: 30 }, { type: 'step', value: 1}] }]},
          ...
          outerRadius: {controls: [{ name: 'outerRadius', label: 'outer radius', type: 'text',  value: Math.round(klass.get('outerRadius')) },                   { name: 'outerRadius', type: 'range', value: Math.round(klass.get('outerRadius')),                   attrs:[{ type: 'min', value: 0 }, {type: 'max', value: stage.width }, { type: 'step', value: 1}] }]},
